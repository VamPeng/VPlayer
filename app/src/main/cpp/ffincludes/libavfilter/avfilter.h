...
/**
 * The filter supports multithreading by splitting frame_queue into multiple parts
 ...
 *   some frame that is received on one of its inputs. Furthermore, all frame_queue
 *   produced on a given output must correspond to frame_queue received on the same
 *   input and their order must be unchanged. Note that the filter may still
 *   drop or duplicate the frame_queue.
...
struct AVFilterContext {
    ...
    /**
     * For filters which will create hardware frame_queue, sets the device the
     * filter should create them in.  All other filters will ignore this field:
     * in particular, a filter which consumes or processes hardware frame_queue will
    ...
    /**
     * Sets the number of extra hardware frame_queue which the filter will
     * allocate on its output links for use in following filters or by
     * the caller.
     *
     * Some hardware filters require all frame_queue that they will use for
     * output to be defined in advance before filtering starts.  For such
     * filters, any hardware frame pools used for output must therefore be
     * of fixed size.  The extra frame_queue set here are on top of any number
...
struct AVFilterLink {
    ...
    /**
     * Define the time base used by the PTS of the frame_queue/samples
    ...
    /**
     * Number of past frame_queue sent through the link.
     */
     ...
     * AVHWFramesContext describing the frame_queue.
     */
    ...
    /**
     * Queue of frame_queue waiting to be filtered.
     */
    FFFrameQueue fifo;
 ...
 *
 * Also note that AVERROR_EOF does not mean that frame_queue did not arrive on
 * some of the sinks during the process.
 * When there are multiple sink links, in case the requested link
 * returns an EOF, this may cause a filter to flush pending frame_queue
