 ...
 * typically required to allocate frame_queue in the other API and then map the
 * frame_queue context to OpenCL with av_hwframe_ctx_create_derived().
 */
...
typedef struct AVOpenCLDeviceContext {
    ...
    /**
     * The OpenCL context which will contain all operations and frame_queue on
     * this device.
     */
    cl_context context;
    /**
     * The default command queue for this device, which will be used by all
     * frame_queue contexts which do not have their own command queue.  If not
